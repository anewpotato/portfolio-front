...
export default function ProjectsDetail({
  ...
  const handleOnNextClick = () =>
    setSelectedProject((prev) => (prev + 1) % projectsInformation.length);

  const handleOnPrevClick = () =>
    setSelectedProject(
      (prev) =>
        (prev - 1 + projectsInformation.length) % projectsInformation.length,
    );
  ...
  const getCardPosition = useCallback(
    (index: number) => {
      ...
      if (index === selectedProject) return 'center';
      if (index === (selectedProject + 1) % projectsInformation.length)
        return 'next';
      if (
        index ===
        (selectedProject - 1 + projectsInformation.length) %
          projectsInformation.length
      )
        return 'prev';
      return 'hidden';
    },
    [selectedProject, windowWidth],
  );
      ...
      <div className="lg:relative w-full h-full flex justify-center items-center">
        ...
        <AnimatePresence>
          {projectsInformation.map(
            ({ id, title, skills, progress, image }, index) => {
